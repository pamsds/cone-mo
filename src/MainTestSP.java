...
public class MainTestSP {
	...
	public static void main(String[] args) throws ClassNotFoundException, IOException, JMException {
		...
		for (int i = 0; i < 1; i++) {
			...
			recordMetric("metrics/SPREAD_nsga.txt", indicators.getSpread(populationSelect));
			populationSelect.printObjectivesToFile("NSGA.txt");
			
			
			population = nsgaCone(problem);
			recordMetric("metrics/SPREAD_cone.txt", indicators.getSpread(population));
			population.printObjectivesToFile("NSGAPC.txt");
		
//			population = nsgaCone2(problem,populationSelect);
//			recordMetric("metrics/SPREAD_cone2.txt", indicators.getSpread(population));
